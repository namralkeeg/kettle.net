...
namespace Kettle.Tests.Sorting
{
    public class QuickSortTests
    {
        ...
        internal static ISort<int> defaultSort = new QuickSort<int>();
        internal static ISort<int> customSort = new QuickSort<int>(new SortCustomComparer());
        ...
        [Fact]
        public void Sort_StressTestDefaultCompareParallel()
        {
            var random = new Random();
            int nodes = 10000;
            var randomList = Enumerable.Range(0, nodes)
            ...
            ((QuickSort<int>)defaultSort).InParallel = true;
            defaultSort.Sort(randomList);
            for (int i = 0; i < nodes; i++)
            {
                Assert.Equal(i, randomList[i]);
        ...
        [Fact]
        public void Sort_StressTestCustomCompareParallel()
        {
            var random = new Random();
            int nodes = 10000;
            var randomList = Enumerable.Range(0, nodes)
